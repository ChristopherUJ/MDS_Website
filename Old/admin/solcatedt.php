...
<?
	...
	if (isset($_POST["cg"]))
	{
		//dispPost();
		$err = false;
		if ($_POST["solcatTitle"] == "" || $_POST["limgge"] == "" || $_POST["solcatDesc"] == "")
			$err = true;
		
		if ($err != true)
		{
			$vals = array();
			$vals[] = "'".$_POST["solcatTitle"]."'";
			$vals[] = "'".$_POST["limgge"]."'";
			$vals[] = "'".$_POST["solcatDesc"]."'";
			if (!$g_db)
				db_connect();
			if (!$g_db)
				...
				if (!isset($_POST["edt"])) {
					$res3 = insert_query("tblsolutioncat",array("solcatTitle","solcatImage","solcatDesc"),$vals);
				}
				else
				$res3 = update_query("tblsolutioncat",array("solcatTitle","solcatImage","solcatDesc"),$vals,"solcatID = ".$_POST["edt"]);
				if ($res3 == false)
				{
				...
				?>
					<script>window.location = "index.php?con=sol&idc=<? echo $theid; ?><? if (isset($_POST["edt"])) echo "&edt=1"; ?>";</script>
				<?
				}
			}
		}
	...
	if (isset($_GET["id"]))
	{
			...
			if (!$g_db)
				...
				$res3 = select_query(array("tblsolutioncat"),array("*"),"solcatID = ".$_GET["id"]);
				if ($res3 == false)
				{
					echo "<h3>System Error. Please try again or come back later. </h3>";
				}
...
<h1 style="font-size:16px; margin-bottom:15px;"><? if ($gedit) echo "Updating Solution Category"; else { ?>
  Add Solution Category
  <? } ?>
  :</h1>

<form action="index.php?con=solcatedt" method="post" name="groupsubmitform" id="groupsubmitform" onsubmit="">
  <? if ($gedit) { ?>
  ...
  <table border="0" align="center" cellpadding="5" style="padding:10px; width:100%;" cellspacing="0">
    <tr>
      <td width="20%" style="padding-top:10px;">&nbsp;</td>
      <td width="85%">&nbsp;</td>
    </tr>
    <tr>
      <td  style="padding-top:10px;" width="20%" valign="top"><strong>Title :</strong></td>
      <td width="85%"  style="padding-top:10px;"><input name="solcatTitle" type="text" id="solcatTitle"  value="<? if (isset($_POST["solcatTitle"])) echo  $_POST["solcatTitle"]; else if ($gedit) echo $row["solcatTitle"];?>" size="60" maxlength="50" />
        (max 50 characters)
        <? if (isset($_POST["solcatTitle"]) && $_POST["solcatTitle"] == "" ) { ?>
        <br />
    ...
    <tr>
      <td width="20%" valign="top" style="border-top:1px solid #CCCCCC;">&nbsp;</td>
...
<br />
<div id="theim" align="center"><img style="border:1px solid #999999;" src="<? if ($row["solcatImage"] == NULL) echo "../images/noimage.jpg"; else echo "../".$row["solcatImage"]; ?>" /><input type="hidden" id="limgge"  name="limgge" value="<? if ($row["solcatImage"] == NULL) echo "../images/noimage.jpg"; else echo $row["solcatImage"]; ?>" /></div></center>
                            <input type="hidden" id="ntplistImage" value="<?php
		if ($row["solcatImage"] != NULL)
			echo $row["solcatImage"];	   	   
...
<table width="100%"><tr><td>                  <p><strong>Discription :</strong></p>
      <p>Use the editor for description of the Solution Category.        </p>
        </p>
  <p>&nbsp;</p>
          </div>
				  <!-- TinyMCE -->
...
<!-- /TinyMCE -->
				  <textarea id="solcatDesc" name="solcatDesc" style="overflow:auto; width:100%" rows="35" cols="100"><?php
if ($row["solcatDesc"] != NULL)
	echo  $row["solcatDesc"];
else
	echo "Category Description goes here!";
 ?></textarea></td></tr>
  <tr>
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td  style="padding-top:10px; border-top:1px solid #CCCCCC;"><input type="submit" name="Submit" value="<? if (isset($_GET["id"])) echo "Update"; else { ?>Add<? }?> Category" /></td>
