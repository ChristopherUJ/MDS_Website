...
<?
	...
	if (isset($_POST["cg"]))
	{
		//dispPost();
		$err = false;
		if ($_POST["solTitle"] == "" || $_POST["solCat"] == "" || $_POST["limgge"] == "" || $_POST["solContent"] == "")
			$err = true;
		...
		if ($err != true)
		{
			...
			if (!$g_db)
				...
				if (!isset($_POST["edt"])) {
					$res3 = insert_query("tblsolutions",array("solTitle","solCat","solImage","solContent"),$vals);
				}
				else
				$res3 = update_query("tblsolutions",array("solTitle","solCat","solImage","solContent"),$vals,"solID = ".$_POST["edt"]);
				if ($res3 == false)
				{
				...
				?>
					<script>window.location = "index.php?con=sol&id=<? echo $theid; ?><? if (isset($_POST["edt"])) echo "&edt=1"; ?>";</script>
				<?
				}
			}
		}
	...
	if (isset($_GET["id"]))
	{
			...
			if (!$g_db)
				...
				$res3 = select_query(array("tblsolutions"),array("*"),"solID = ".$_GET["id"]);
				if ($res3 == false)
				{
					echo "<h3>System Error. Please try again or come back later. </h3>";
				}
...
<h1 style="font-size:16px; margin-bottom:15px;"><? if ($gedit) echo "Updating Solution/Service Details"; else { ?>
  Add Solution/Service
  <? } ?>
  :</h1>

<form action="index.php?con=soledt" method="post" name="groupsubmitform" id="groupsubmitform" onsubmit="">
  <? if ($gedit) { ?>
  ...
  <table border="0" align="center" cellpadding="5" style="padding:10px; width:100%;" cellspacing="0">
    <tr>
      <td width="40%" style="padding-top:10px;">&nbsp;</td>
      <td width="85%">&nbsp;</td>
    </tr>
    <tr>
      <td  style="padding-top:10px;" width="40%" valign="top"><strong>Title :</strong></td>
      <td width="85%"  style="padding-top:10px;"><input name="solTitle" type="text" id="solTitle"  value="<? if (isset($_POST["solTitle"])) echo  $_POST["solTitle"]; else if ($gedit) echo $row["solTitle"];?>" size="60" maxlength="50" />
        (max 50 characters)
        <? if (isset($_POST["solTitle"]) && $_POST["solTitle"] == "" ) { ?>
        <br />
    ...
    <tr>
      <td width="40%" style="border-top:1px solid #CCCCCC;" valign="top">&nbsp;</td>
      <td width="85%"  style="border-top:1px solid #CCCCCC;" >&nbsp;</td>
    </tr>
    <tr>
      <td  style="padding-top:10px;" valign="top"><strong>Category :</strong></td>
      <td  style="padding-top:10px;"><label>
        <select name="solCat" id="solCat">
		<?
		if (!$g_db)
				db_connect();
			if (!$g_db)
				...
				$resc = select_query(array("tblsolutioncat"), array("*"), "1 = 1");
...
<? }
				...
				{
		?>
          <option <? if ($rowc["solcatID"] == $row["solCat"]) { ?>selected="selected"<? }?> value="<? echo $rowc["solcatID"]; ?>"><? echo $rowc["solcatTitle"]; ?></option>
		<? }
		}
	} ?>
        </select>
    ...
    <tr>
      <td style="border-top:1px solid #CCCCCC;" valign="top">&nbsp;</td>
      <td  style="border-top:1px solid #CCCCCC;" >&nbsp;</td>
    </tr>
    <tr>
      <td width="40%" valign="top"><div align="left">
                    <b>Image:</b>
                </div></td>
...
<br />
<div id="theim" align="center"><img style="border:1px solid #999999;" src="<? if ($row["solImage"] == NULL) echo "../images/noimage.jpg"; else echo "../".$row["solImage"]; ?>" /><input type="hidden" id="limgge"  name="limgge" value="<? if ($row["solImage"] == NULL) echo "../images/noimage.jpg"; else echo $row["solImage"]; ?>" /></div></center>
                            <input type="hidden" id="ntplistImage" value="<?php
		if ($row["solImage"] != NULL)
			echo $row["solImage"];	   	   
	   ?>" onchange="ajaxpage('savimg.php?id=<? echo $row["solID"]; ?>&img='+this.value,'theim');" /></td>
    ...
    <tr>
      <td valign="top"  style="border-top:1px solid #CCCCCC;" >&nbsp;</td>
      <td  style="border-top:1px solid #CCCCCC;" >&nbsp;</td>
    </tr>
  </table>
<table width="100%"><tr><td>                  <p><strong>Page Content :</strong></p>
      <p>Use the editor for content of the Service.        </p>
        </p>
  <p>&nbsp;</p>
          </div>
				  <!-- TinyMCE -->
...
<!-- /TinyMCE -->
				  <textarea id="solContent" name="solContent" style="overflow:auto; width:100%" rows="35" cols="100"><?php
if ($row["solContent"] != NULL)
	echo  $row["solContent"];
else
	echo "Solution Information goes here!";
 ?></textarea></td></tr>
  <tr>
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td  style="padding-top:10px; border-top:1px solid #CCCCCC;"><input type="submit" name="Submit" value="<? if (isset($_GET["id"])) echo "Update"; else { ?>Add<? }?> Service" /></td>
