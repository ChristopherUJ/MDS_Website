	...
	function onUpload(&$man, $cmd, $input) {
		if ($cmd  == "upload") {
			...
			if (isset($_FILES["Filedata"])) {
				$_FILES["file0"]["name"] = strtolower($_FILES["Filedata"]["name"]);
				...
				$input["name0"] = strtolower(substr($_FILES["Filedata"]["name"], 0, strrpos($_FILES["Filedata"]["name"], ".")));
			}
			...
			if ($man->verifyPath($path)) {
				...
				for ($i=0; isset($_FILES['file' . $i]['tmp_name']); $i++) {
					$filename = strtolower(utf8_encode($input["name" . $i]));
