...
<?
	...
	if (isset($_POST["cg"]))
	{
		//dispPost();
		$err = false;
		if ($_POST["projTitle"] == "" || $_POST["projContent"] == "")
			$err = true;
		
		if ($err != true)
		{
			$vals = array();
			$vals[] = "'".$_POST["projTitle"]."'";
			$vals[] = "'".$_POST["projContent"]."'";
			if (!$g_db)
				db_connect();
			if (!$g_db)
				...
				if (!isset($_POST["edt"])) {
					$res3 = insert_query("tblprojects",array("projTitle","projContent"),$vals);
				}
				else
				$res3 = update_query("tblprojects",array("projTitle","projContent"),$vals,"projID = ".$_POST["edt"]);
				if ($res3 == false)
				{
				...
				?>
					<script>window.location = "index.php?con=proj&id=<? echo $theid; ?><? if (isset($_POST["edt"])) echo "&edt=1"; ?>";</script>
				<?
				}
			}
		}
	...
	if (isset($_GET["id"]))
	{
			...
			if (!$g_db)
				...
				$res3 = select_query(array("tblprojects"),array("*"),"projID = ".$_GET["id"]);
				if ($res3 == false)
				{
					echo "<h3>System Error. Please try again or come back later. </h3>";
				}
...
<h1 style="font-size:16px; margin-bottom:15px;"><? if ($gedit) echo "Updating Project Details"; else { ?>
  Add Project
  <? } ?>
  :</h1>

<form action="index.php?con=projedt" method="post" name="groupsubmitform" id="groupsubmitform" onsubmit="">
  <? if ($gedit) { ?>
  ...
  <table border="0" align="center" cellpadding="5" style="padding:10px; width:100%;" cellspacing="0">
    ...
    <tr>
      ...
      <td width="85%"  style="padding-top:10px;"><input name="projTitle" type="text" id="projTitle"  value="<? if (isset($_POST["projTitle"])) echo  $_POST["projTitle"]; else if ($gedit) echo $row["projTitle"];?>" size="60" maxlength="50" />
        (max 50 characters)
        <? if (isset($_POST["projTitle"]) && $_POST["projTitle"] == "" ) { ?>
        <br />
...
<table width="100%"><tr><td>                  <p><strong>Project Description  :</strong></p>
      <p>Use the editor for description of th project.</p>
        </p>
  <p>&nbsp;</p>
          </div>
				  <!-- TinyMCE -->
...
<!-- /TinyMCE -->
				  <textarea id="projContent" name="projContent" style="overflow:auto; width:100%" rows="35" cols="100"><?php
if ($row["projContent"] != NULL)
	echo  $row["projContent"];
else
 ...
 ?></textarea></td></tr>
  <tr>
    <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td  style="padding-top:10px; border-top:1px solid #CCCCCC;"><input type="submit" name="Submit" value="<? if (isset($_GET["id"])) echo "Update"; else { ?>Add<? }?> Project" /></td>
