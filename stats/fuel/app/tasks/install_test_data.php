...
/**
 * Description of Install_test_data
 *
 * @author David Weber
 */
class Install_test_data {

    public static function run() {
        \Fuel\Core\Cli::write('Task - Install Core DB: Started', 'white');
        $core_data = \Fuel\Core\File::read(APPPATH . 'sql' . DS . 'test_data.sql', true);
        ...
        try {
            ...
            \Fuel\Core\Cli::write('Task - Install Test Data: Successfully completed', 'green');
        } catch (\Fuel\Core\Database_Exception $ex) {
            \Fuel\Core\DB::rollback_transaction();
            \Fuel\Core\Cli::error('Task - Install Test Data: Database Exception: ' . $ex->getMessage() . ' Statement: ' . $insert_statement);
        }
    }
